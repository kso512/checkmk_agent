...
for distro in ${distros[@]}; do
  ...
  echo "--- Testing playbook for no failures in distro: ${distro}"
  ...
  echo "--- Testing playbook for no changes (idempotence) in distro: ${distro}"
  ansible_output=$(docker exec --tty "${container}" env TERM=xterm \
      ansible-playbook /etc/ansible/roles/role_under_test/test/playbook.yml)
  echo "${ansible_output}" \
      | grep -o "changed=0"
  ...
  #     checkmk_agent_home
  #     checkmk_agent_user
  echo "--- Testing CheckMK agent in distro: $distro"
